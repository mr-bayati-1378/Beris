  ...
  try {
    ...
    await Promise.all(
      productsData.map((product, index) => {
        // تخفیف رندوم برای برخی محصولات
        const hasDiscount = Math.random() < 0.3; // 30% احتمال تخفیف
        const discountPercent = hasDiscount ? Math.floor(Math.random() * 40) + 10 : undefined; // بین 10 تا 50 درصد
        ...
        return prisma.product.create({
          data: {
            ...
            hasDiscount,
            discountPercent,
            discountStartDate: hasDiscount ? new Date() : undefined,
            discountEndDate: hasDiscount ? new Date(Date.now() + 30 * 24 * 60 * 60 * 1000) : undefined, // 30 روز
            categoryL3Id: l3s[product.categoryL3Id].id,
          },
        });
      })
    );
